<?php namespace PhpNFe\NFSe\Providers\SimplissAbrasf_v2_03;

use Carbon\Carbon;
use PhpNFe\NFSe\NFSe;
use Illuminate\Support\Arr;
use PhpNFe\NFSe\Builder\Rps;
use PhpNFe\NFSe\EmpresaNFSe;
...
class SimplissProvider extends Provider
{
     ...
     * @param NFSe $nfse
     */
    public function __construct(NFSe $nfse, EmpresaNFSe $empresa)
    {
        parent::__construct($nfse, $empresa);

        $this->portalUser = $empresa->config('portal.user', '');
        $this->portalPass = $empresa->config('portal.pass', '');
    }
    ...
    public function autorizar(Rps $rps, $amb)
    {
        return $this->callWithCertPath(function($certPath) use ($rps, $amb) {
            ...
            $rps_xml = file_get_contents(__DIR__ . '/rps2.xml');
            $data = $rps_xml;

            //file_put_contents(__DIR__ . '/envio_rps.json', json_encode($data));
            $response = $soap->$method($data);
            //$response = $soap->$method($data);
            //$request = $soap->__getLastRequest();
            //$header_request = $soap->__getLastRequestHeaders();
            //file_put_contents(__DIR__ . '/envio_rps.xml', $request);

            return new RetornoAutoriza($response);
        });
    }
